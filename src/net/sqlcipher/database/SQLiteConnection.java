 ...
 * a native <code>sqlite3</code> object.
 * <p>
 ...
 * concurrently access the database. Without WAL, readers and writers are
 * mutually exclusive.
 * </p>
 *
 ...
 * perform a database operation and are then returned to the pool. At any given
 * time, a connection is either owned and used by a {@link SQLiteSession} object
 * or the {@link SQLiteConnectionPool}. Those classes are responsible for
 * serializing operations to guard against concurrent use of a connection.
 * </p>
 * <p>
...
//        }
    ...
    /**
     * Prepares a statement for execution but does not bind its parameters or
     * execute it.
     * <p>
     * This method can be used to check for syntax errors during compilation
     * prior to execution of the statement. If the {@code outStatementInfo}
     * argument is not null, the provided {@link SQLiteStatementInfo} object is
     * populated with information about the statement.
     * </p>
     * <p>
     * A prepared statement makes no reference to the arguments that may
     * eventually be bound to it, consequently it it possible to cache certain
     * prepared statements such as SELECT or INSERT/UPDATE statements. If the
     * statement is cacheable, then it will be stored in the cache for later.
     * </p>
     * <p>
     * To take advantage of this behavior as an optimization, the connection
     * pool provides a method to acquire a connection that already has a given
     * SQL statement in its prepared statement cache so that it is ready for
     * execution.
     * </p>
     *
     * @param sql
     ...
     *            The {@link SQLiteStatementInfo} object to populate with
     *            information about the statement, or null if none.
     *
     ...
     *
     * @throws SQLiteException
     *             if an error occurs, such as a syntax error or invalid number
     *             of bind arguments.
    ...
    /**
     * Executes a statement that returns a single BLOB result as a file
     * descriptor to a shared memory region.
     *
     * @param sql
     *            The SQL statement to execute.
     * @param bindArgs
     ...
     * @param cancellationSignal
     *            A signal to cancel the operation in progress, or null if none.
     * @return The file descriptor for a shared memory region that contains the
     *         value of the first column in the first row of the result set as a
     *         BLOB, or null if none.
     *
     * @throws SQLiteException
     *             if an error occurs, such as a syntax error or invalid number
     *             of bind arguments.
    ...
    /**
     * Executes a statement that returns the row id of the last row inserted by
     * the statement. Use for INSERT SQL statements.
     *
     * @param sql
     *            The SQL statement to execute.
     * @param bindArgs
     ...
     * @param requiredPos
     *            The position of a row that MUST be in the window. If it won't
     *            fit, then the query should discard part of what it filled so
     *            that it does. Must be greater than or equal to
     *            <code>startPos</code>.
     * @param countAllRows
     *            True to count all rows that the query would return regagless
     *            of whether they fit in the window.
     * @param cancellationSignal
     *            A signal to cancel the operation in progress, or null if none.
     * @return The number of rows that were counted during query execution.
     *         Might not be all rows in the result set unless
     *         <code>countAllRows</code> is true.
     *
     * @throws SQLiteException
     *             if an error occurs, such as a syntax error or invalid number
     *             of bind arguments.
...
//        }
     ...
     *
     * This function is written so that it may be called by a thread that does
     * not own the connection. We need to be very careful because the connection
     * state is not synchronized.
     *
     * At worst, the method may return stale or slightly wrong data, however it
     * should not crash. This is ok as it is only used for diagnostic purposes.
     *
     * @param printer
     *            The printer to receive the dump, not null.
     * @param verbose
    ...
    /**
     * Describes the currently executing operation, in the case where the caller
     * might not actually own the connection.
     *
     * This function is written so that it may be called by a thread that does
     * not own the connection. We need to be very careful because the connection
     * state is not synchronized.
     ...
     * should not crash. This is ok as it is only used for diagnostic purposes.
     *
     * @return A description of the current operation including how long it has
     *         been running, or null if none.
     */
    ...
    /**
     * Collects statistics about database connection memory usage, in the case
     * where the caller might not actually own the connection.
     *
     ...
     * does not have a finalizer. This is deliberate. The
     * {@link SQLiteConnection} owns the statement object and will take care of
     * freeing it when needed. In particular, closing the connection requires a
     * guarantee of deterministic resource disposal because all native statement
     * objects must be freed before the native database object can be closed. So
     * no finalizers here.
     */
