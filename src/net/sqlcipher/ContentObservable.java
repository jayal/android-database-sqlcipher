...
/**
 * A specialization of {@link Observable} for {@link ContentObserver} that
 * provides methods for sending notifications to a list of
 * {@link ContentObserver} objects.
 */
public class ContentObservable extends android.database.ContentObservable {
     ...
     * <p>
     * If <code>selfChange</code> is true, only delivers the notification to the
     * observer if it has indicated that it wants to receive self-change
     * notifications by implementing
    ...
    /**
     * Invokes {@link ContentObserver#dispatchChange(boolean, Uri)} on each
     * observer. Includes the changed content Uri when available.
     * <p>
     * If <code>selfChange</code> is true, only delivers the notification to the
     * observer if it has indicated that it wants to receive self-change
     * notifications by implementing
    ...
    public void dispatchChange(boolean selfChange, Uri uri) {
        synchronized (mObservers) {
            for (android.database.ContentObserver observer : mObservers) {
