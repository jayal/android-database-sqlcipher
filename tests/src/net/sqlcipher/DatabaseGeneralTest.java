...
import static net.sqlcipher.DatabaseUtils.InsertHelper.TABLE_INFO_PRAGMA_COLUMNNAME_INDEX;
import static net.sqlcipher.DatabaseUtils.InsertHelper.TABLE_INFO_PRAGMA_DEFAULT_INDEX;
...
public class DatabaseGeneralTest extends AndroidTestCase implements PerformanceTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        mDatabaseFile = new File(dbDir, "db_general_test.db");
    ...
    public void testPhoneNumbersEqual() throws Exception {
         ...
         * assertNotNull(c); assertEquals(1, c.getCount()); c.moveToFirst();
         * number = c.getString(c.getColumnIndexOrThrow("num"));
         * assertEquals("+" + PHONE_NUMBER, number); c.close();
         */
    ...
    /**
     * This test is available only when the platform has a locale with the
     * language "ja". It finishes without failure when it is not available.
     */
    @MediumTest
